...
";
$pendingVerifications = mysqli_query($conn, $query);
...
";
$userReports = mysqli_query($conn, $query);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['verification_action'])) {
    ...
    ";
    $updateVerification = mysqli_query($conn, $updateQuery);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['acknowledge_report'])) {
    $reportId = mysqli_real_escape_string($conn, $_POST['report_id']);  // Sanitize the report ID to prevent SQL injection
    mysqli_query($conn, "UPDATE notifications SET is_read = TRUE WHERE id = '$reportId'");  // Execute the update query
    echo "<script>alert('Report acknowledged successfully.'); window.location.href = 'notifications.php';</script>";  // Provide feedback and redirect
}
